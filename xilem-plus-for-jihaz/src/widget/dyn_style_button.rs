...
use masonry::core::{
    AccessCtx, AccessEvent, Action, ArcStr, BoxConstraints, BrushIndex, EventCtx, LayoutCtx, PaintCtx, PointerButton, PointerEvent, PropertiesMut, PropertiesRef, QueryCtx, TextEvent, Update, UpdateCtx, Widget, WidgetId, WidgetMut, WidgetPod
};
use masonry::kurbo::{Insets, Size};
use masonry::theme;
use masonry::util::{UnitPoint, fill_lin_gradient, stroke};

use crate::text::GeneralTextStyles;
use crate::theme::{color, distance};
...
///
#[doc = masonry::include_screenshot!("widget/screenshots/masonry__widget__button__tests__hello.png", "Button with text label.")]
pub struct Button {
...
impl Button {
    ...
    /// ```
    pub fn new(text: impl Into<ArcStr>, general_styles: GeneralTextStyles<BrushIndex>) -> Self {
        Self::from_label(Label::new(text, general_styles))
    }
...
impl Widget for Button {
    ...
    fn paint(&mut self, ctx: &mut PaintCtx, _props: &PropertiesRef<'_>, scene: &mut Scene) {
        ...
        let size = ctx.size();
        let stroke_width = distance::BUTTON_BORDER_WIDTH;

        let rounded_rect = size
            .to_rect()
            .inset(-stroke_width / 2.0)
            .to_rounded_rect(distance::BUTTON_BORDER_RADIUS);

        let brush = if ctx.is_disabled() {
            color::default_dark::BACKGROUND
        } else if is_active {
            color::default_dark::CLICKED
        } else if is_hovered {
            color::default_dark::HOVERED
        } else {
            color::default_dark::ITEM
        };

        let border_color = if is_hovered && !ctx.is_disabled() {
            color::default_dark::BORDER_HOVERED
        } else {
            color::default_dark::BORDER
        };
