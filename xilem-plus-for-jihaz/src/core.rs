...
use masonry::{app::{AppDriver, MasonryState}, core::{Action, Widget}};
use winit::window::WindowAttributes;
use xilem::{Color, EventLoop};
use xilem_core::Message;

/// Initlizes masonry state and tracing. Then you can use the masonry state
/// along with the app driver to build your [`winit::application::ApplicationHandler`] 
/// type, and after that, you can run it with [`masonry::app::EventLoop::run_app`].
/// 
/// This allows your app to embed a Xilem application from a pre-existing Winit event loop.
/// 
/// This is similar to [`masonry::app::run_with`], but it returns [`MasonryState`]
/// after initializing it using the [`xilem::MasonryDriver::on_start`] method.
pub fn initialize_state_and_tracing<'a>(
    event_loop: &EventLoop,
    window: WindowAttributes,
    root_widget: impl Widget,
    app_driver: &mut (impl AppDriver + 'static),
    ...
    let _ = tracing_backend::try_init_tracing();

    let mut masonry_state = MasonryState::new(window, &event_loop, root_widget, background_color);
    app_driver.on_start(&mut masonry_state);
    masonry_state
}

/// A trait that accesses the concrete type [`Action`] of the [`DynMessage`] and then the inner concrete type of the [`Action::Other`] varient.
/// 
/// This only succeeds if the [`DynMessage`] is an [`Action`] type of varient `Other`, and the [`Action::Other`] varient is of type `A`.
pub trait DowncastActionMessageOther<A: Any + Send> {
    /// Accesses the concrete type [`Action`] of this [`DynMessage`] and then the inner concrete type of the [`Action::Other`] varient.
    /// 
    /// This only succeeds if the [`DynMessage`] is an [`Action`] type of varient `Other`, and the [`Action::Other`] varient is of type `A`.
    ///
    ...
    /// ## Errors
    ///
    /// If the message contained within `self` is not of type [`Action`] of varient [`Action::Other`]
    /// and the varient inner value is not of type `A`, then this method returns `self`
    /// (so that e.g. a different type can be used)
    fn downcast(self: Box<Self>) -> Result<Box<A>, Box<Self>>;
}

impl<A: Any + Send> DowncastActionMessageOther<A> for dyn Message {
    
    fn downcast(self: Box<Self>) -> Result<Box<A>, Box<Self>> {

        if self.deref().as_any().is::<Action>() {
...
/// Re-exported from [`masonry::app::tracing_backend`], because it's private.
mod tracing_backend {
