...
macro_rules! generate_directory_function {
    ...
    //     )+
    // };
    // For generates a function that creates the directories::ProjectDir to be used for your app.
    (
        $(#[$($attr:meta)*])*
        project
        $qualifier:literal
...
macro_rules! project_directory {
    (
        ...
        /// Creates a directories::ProjectDirs from the given path.
        #[cfg(not(feature = "portable"))]
        fn project_dirs() -> Option<directories::ProjectDirs> {
            directories::ProjectDirs::from($qualifier, $organisation, $app_name)
        }
    
        /// Creates a directories::ProjectDirs, return path adjacent to app executable when built as portable.
        #[cfg(feature = "portable")]
        fn project_dirs() -> Option<directories::ProjectDirs> {
            if let Ok(current_exe) = std::env::current_exe() {
                if let Some(parent) = current_exe.parent() {
                    return directories::ProjectDirs::from_path(parent.join($data_local_folder_name));
                }
